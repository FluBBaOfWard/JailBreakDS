...
//	vlm5030Chip = memUncached(vlm5030_start(0));
	vlm5030Chip = vlm5030_start(0);
	setupGraphics();

	irqSet(IRQ_VBLANK, myVblank);
	fifoSendAddress(FIFO_USER_08, &ipcData);
	fifoSendAddress(FIFO_USER_07, &sn76496_0);
	fifoSendAddress(FIFO_USER_06, vlm5030Chip);
	machineInit();
	setupGUI();
	getInput();
	loadCart(0,0);
...
//---------------------------------------------------------------------------------
	...
	REG_BG0CNT = BG_64x32 | BG_COLOR_16  | BG_MAP_BASE(0) | BG_TILE_BASE(2) | BG_PRIORITY(2);
	REG_BG1CNT = BG_64x32 | BG_COLOR_256 | BG_MAP_BASE(2) | BG_TILE_BASE(1) | BG_PRIORITY(2);
